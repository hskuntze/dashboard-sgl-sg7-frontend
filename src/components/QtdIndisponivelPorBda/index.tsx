...
const QtdIndisponivelPorBda = ({ selectedData }: Props) => {
  const { data, loading } = useFetchData<QtdIndisponivelPorBdaType>({
  ...
  const options: ApexOptions = {
    ...
    colors: ["#7300F5", "#7F16DB", "#7927C2", "#7132A8", "#67398F", "#5A3B75", "#4B375C", "#392E42", "#2E2933", "#4C4850"],
    stroke: {
      show: false,
    },
    plotOptions: {
    ...
    <div className="card-chart">
      {loading ? (
        ...
        <div className="severity-column-chart">
          <ReactApexChart options={options} series={values} type="pie" height={400} width={400} />
        </div>
      )}
    </div>
  );
