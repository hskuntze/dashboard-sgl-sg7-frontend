...
const ExecucaoConjuntoCFF = ({ selectedData }: Props) => {
  const { data, loading } = useFetchData<ExecucaoPagoCFF>({
    url: "/cff/execucao/conjunto",
  ...
  const options: ApexOptions = {
    ...
    colors: ["#009B3A", "#FEDF00", "#023373", "#A30016", "#FF9C04", "#78D952", "#A752D9"], // Cores para cada série
    legend: {
      show: false,
    },
  };
  ...
  return (
    <div className="card-chart">
      {loading ? (
        ...
        <ReactApexChart options={options} series={series} type="bar" height={300} width={700} />
      )}
    </div>
  );
};

export default ExecucaoConjuntoCFF;
