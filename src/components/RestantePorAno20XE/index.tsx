...
import { RestosPagarAno20XEType } from "types/relatorio/restospagarano20xe";

interface Props {
  onSelectItem: (element: number) => void;
}

const RestantePorAno20XE = ({ onSelectItem }: Props) => {
  const { data, loading } = useFetchData<RestosPagarAno20XEType>({
    url: "/acao20xe/restos",
    initialData: null,
  });
  ...
  const values = data.map((item) => item.restosPagar);
  ...
  const options: ApexOptions = {
    ...
    labels: labels.map((num) => num.toString()),
    colors: colors,
    stroke: {
      show: false,
    },
    ...
    <div className="card-chart">
      {loading ? (
        ...
        <div className="severity-column-chart">
          <ReactApexChart options={options} series={values} type="donut" height={elementSize.height} width={elementSize.width} />
        </div>
      )}
    </div>
  );
...
export default RestantePorAno20XE;
